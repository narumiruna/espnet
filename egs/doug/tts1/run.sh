...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/data_prep.sh ${db_root}/doug data/${trans_type}_train ${trans_type}
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    # make a dev set
    utils/subset_data_dir.sh --last data/${trans_type}_train 676 data/${trans_type}_deveval
    utils/subset_data_dir.sh --last data/${trans_type}_deveval 84 data/${eval_set}
    utils/subset_data_dir.sh --first data/${trans_type}_deveval 84 data/${dev_set}
    n=$(( $(wc -l < data/${trans_type}_train/wav.scp) - 676 ))
